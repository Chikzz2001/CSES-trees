...
int l[200005],r[200005],z[200005];

pair<int,int>  dfs1(vector<vector<int>>& g,int node,int p,int tot)
{
    l[node]+=tot+l[p];
    int x=0,y=0;
    for(auto child:g[node])
    {
        if(child!=p)
        {
            pair<int,int> P=dfs1(g,child,node,tot+y+1);
            l[node]+=P.ss+P.ff;
            x+=P.ss+P.ff;
            y+=P.ss;
        }
...
pair<int,int> dfs2(vector<vector<int>>& g,int node,int p,int tot)
{
    r[node]+=tot+r[p];
    int x=0,y=0;
    for(int i=g[node].size()-1;i>=0;i--)
    {
        ...
        if(child!=p)
        {
            pair<int,int> P=dfs2(g,child,node,tot+y);
            r[node]+=P.ss+P.ff;
            x+=P.ss+P.ff;
            y+=P.ss;
        }
